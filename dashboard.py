...
def create_byage_df(df):
    ...
    byage_df['age_group'] = pd.Categorical(
        byage_df['age_group'], ["Youth", "Adults", "Senior"])
    return byage_df
...
with st.sidebar:
    ...
    # mengambil start_date & end_date daru date_user
...
ax.plot(
    daily_orders_df["order_date"], daily_orders_df["order_count"], marker="o", linewidth=2, color="#90CAF9"
)
...
ax[0].set_title('Best Recency (days)', loc='center', fontsize=50)
...
ax[1].set_title('Best Frequency', loc='center', fontsize=50)
...
ax[2].set_title('Best Monetary', loc='center', fontsize=50)
